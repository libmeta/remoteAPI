...
int DeviceVideoCapturer::GetDeviceIndex(const std::string& device)
{
    ...
    for (int i = 0; i < num_devices; ++i) {
        ...
        if (info->GetDeviceName(static_cast<uint32_t>(i), name, kSize, mid,
            ...
            if (candidate_mid.size() >= size && std::equal(std::begin(device), std::end(device), std::begin(candidate_mid))) {
                return i;
            }
            std::string candidate_name { name };
            if (candidate_name.size() >= size && std::equal(std::begin(device), std::end(device), std::begin(candidate_name))) {
                return i;
            }
        }
    }
