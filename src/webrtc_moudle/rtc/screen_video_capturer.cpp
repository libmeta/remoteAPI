#include "screen_video_capturer.hpp"
...
bool ScreenVideoCapturer::CaptureProcess()
{
    ...
    int capture_period = std::max((last_capture_duration * 100) / max_cpu_consumption_percentage_,
    ...
    std::unique_ptr<webrtc::DesktopFrame> frame)
{
    ...
    if (!previous_frame_size_.equals(frame->size())) {
        ...
        if (capture_width_ > max_width_) {
            ...
            capture_height_ = frame->size().height() * max_width_ / frame->size().width();
        }
        if (capture_height_ > max_height_) {
            capture_width_ = frame->size().width() * max_height_ / frame->size().height();
    ...
    if (frame->size().width() <= 2 || frame->size().height() <= 1) {
        ...
        if (!frame->size().equals(output_size)) {
            ...
            if ((float)output_size.width() / (float)output_size.height() < (float)frame->size().width() / (float)frame->size().height()) {
                int32_t output_height = frame->size().height() * output_size.width() / frame->size().width();
                ...
                int32_t output_width = frame->size().width() * output_size.height() / frame->size().height();
